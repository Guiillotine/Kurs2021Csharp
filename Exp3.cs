...
namespace Kurs2021Csharp
{
    public partial class Exp3 : Form
    {
        ...
        {
            if (Globals.tableRegZd.GetRowsNum() != 0) this.from.Text = "1"; else this.from.Text = "0";
            this.to.Text = Globals.tableRegZd.GetRowsNum().ToString();
        }
        ...
        private void buttonOk_Click(object sender, EventArgs e)
        {
            ...
            else if (((Convert.ToInt32(this.from.Text) > Globals.tableRegZd.GetRowsNum()) || (Convert.ToInt32(this.from.Text) == 0)) && (f == 1)) 
            { 
            ...
            else if (((Convert.ToInt32(this.to.Text) > Globals.tableRegZd.GetRowsNum()) || (Convert.ToInt32(this.to.Text) == 0)) && (f == 1))  
            { 
                MessageBox.Show("Строки с данным номером не существует", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                f = 0;
            }
            ...
            if (f == 1)
            {
                ...
                Globals.tableRegZd.ExpTable(this.fileName.Text, from, to);
